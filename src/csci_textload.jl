...
* **F**::String or Vector{String} = File or an array of files to load
...
* **select**::Vector{Symbol} (optional) = List of column names as Symbols to be loaded, names not included will not be loaded. [] is default
\n\n
...
function csci_textload(F::String,rootname::String,mindate::DateTime=DateTime(0),maxdate::DateTime=DateTime(9999);headerlines::Int=4,headeroutput::Bool=false,verbose::Bool=true,recur::Int=2,strlist::Vector{String}=String[],intlist::Vector{String}=String[],timelist::Vector{String}=String[],select::Vector{Symbol}=Symbol[])
	...
	# Convert to Vector{String,1}
	temp = Vector{String}(undef,length(files))
	for i=1:1:length(files)
		temp[i] = files[i]
	end
	files = temp
	...
	if headeroutput
		(D,loggerStr,colsStr,unitsStr,processingStr) = csci_textload(files,headerlines=headerlines,headeroutput=headeroutput,verbose=verbose,strlist=strlist,intlist=intlist,timelist=timelist,select=select)
		...
		D = csci_textload(files,headerlines=headerlines,headeroutput=headeroutput,verbose=verbose,strlist=strlist,intlist=intlist,timelist=timelist,select=select)
...
function csci_textload(F::Vector{T};headerlines::Int=4,headeroutput::Bool=false,verbose::Bool=true,strlist::Vector{String}=String[],intlist::Vector{String}=String[],timelist::Vector{String}=String[],select::Vector{Symbol}=Symbol[]) where T <: String
	D = DataFrame[]
	loggerStr = ""
	colsStr = ""
	unitsStr = ""
	...
	for i=1:1:length(F)
		if headeroutput
			(tempD,loggerStr,colsStr,unitsStr,processingStr) = csci_textread(F[i],headerlines=headerlines,headeroutput=headeroutput,verbose=verbose,strlist=strlist,intlist=intlist,timelist=timelist,select=select)
		else
			(tempD) = csci_textread(F[i],headerlines=headerlines,headeroutput=headeroutput,verbose=verbose,strlist=strlist,intlist=intlist,timelist=timelist,select=select)
		end
